...
function SubRnD2({ match, history, isPc }: { match: any; history: any; isPc: boolean }) {
  ...
  const SubRnD2_Mobile = ({ match, history, isPc }: { match: any; history: any; isPc: boolean }) => {
    return (
      <div>Hello!</div>
    )
  }
  return (
    <>
      ...
      {!isPc && <SubRnD2_Mobile match={match} history={history} isPc={isPc} />}
    </>
  );
}
